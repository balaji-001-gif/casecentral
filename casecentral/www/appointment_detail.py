import frappe
from frappe import _

def get_context(context):
    appt_name = frappe.form_dict.get("name")
    if not appt_name:
        frappe.local.flags.redirect_to = "/appointments"
        return

    appt = frappe.get_doc("Customer Appointment", appt_name)
    
    # Permission Check
    user_email = frappe.session.user
    customer = frappe.db.get_value("Customer", {"email_id": user_email}, "name")
    if not customer:
        customer_id = frappe.db.sql("""
            SELECT dl.link_name FROM `tabDynamic Link` dl
            JOIN `tabContact` c ON c.name = dl.parent
            WHERE dl.link_doctype = 'Customer' AND c.email_id = %s
        """, (user_email,))
        if customer_id:
            customer = customer_id[0][0]

    if appt.customer != customer:
        frappe.throw(_("You do not have permission to view this appointment"), frappe.PermissionError)

    context.doc = appt
    
    # Get Attachments
    context.attachments = frappe.get_all("File", 
        filters={"attached_to_doctype": "Customer Appointment", "attached_to_name": appt_name},
        fields=["file_name", "file_url", "creation"],
        ignore_permissions=True
    )
